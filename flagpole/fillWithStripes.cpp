...
void FillWithStripes(NeoPixelBus<NeoGrbFeature, Neo800KbpsMethod> &strip, HslColor colourList[], uint8_t colourCount, HslColor black, uint8_t pixelCount) {
  strip.ClearTo(black);
  strip.Show();
  ...
  for (int stripeNumber = 0; stripeNumber < colourCount; stripeNumber++)
  {
    ...
    {
      strip.SetPixelColor(frameCount - 1, colourList[stripeNumber]);
      strip.Show();
      delay(100);
    }
    delay(500);
